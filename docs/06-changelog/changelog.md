...
This is the consumer-facing release history of the `mad-bros` package, generated by [python-semantic-release](https://python-semantic-release.readthedocs.io/) from Conventional Commits and grouped by Features / Bug Fixes per released version. Only changes in the public scope set — `http`, `sse`, `cli`, `config`, `agents`, `deps` — appear here, per the repository's versioning policy ([CLAUDE.md](../../CLAUDE.md) hard rule 12). Internal `refactor` / `chore` / `test` changes still bump the version per Conventional Commits but are intentionally filtered out of these release notes by the `exclude_commit_patterns` in `[tool.semantic_release]` (`pyproject.toml`). The canonical source is the repo-root [`CHANGELOG.md`](../../CHANGELOG.md); this page mirrors it faithfully and is not a raw `git log`.
