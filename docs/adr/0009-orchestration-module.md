...
| `agent.autosync.rate_limited` | The post-run auto-sync (#8) hit a rate limit *after* the primary run had already succeeded. A non-terminal signal: the primary task is left `task.completed`; the auto-sync is treated as best-effort and not retried (issue #87, §13). Lives in the `agent.*` namespace, not `session.error`/`task.*`. | `{reason}` |
...
### 13. Post-run auto-sync is best-effort under a rate limit (amendment, 2026-06-27 — issue #87)

`_run_launcher` (in `core/sessions/use_cases/send_user_message.py`) runs the primary prompt, then always launches the post-run auto-sync (#8) — a second agent invocation in the same workspace that branches / commits / pushes / opens a PR. That second `launcher.run()` was wrapped in a broad `except Exception` that also caught `RateLimitError`: it emitted `session.error`, stored the exception, and (on the dispatcher path, `propagate_failures=True`) re-raised it at the end of the coroutine. `Dispatcher._run_task`'s `except RateLimitError` then treated it as a retryable *primary* failure and re-ran `_run_launcher` **from the top** — re-executing the already-successful primary prompt (wasted work, and possible duplicate side effects: commits / branches / PRs). This was the follow-up #79 explicitly carved out.

**The primary's success is final; auto-sync is best-effort.** Once the primary run has completed, re-running the coroutine can only repeat work that already succeeded. So a `RateLimitError` raised by the auto-sync invocation is caught in its own `except` branch (ahead of the broad `except Exception`) and is deliberately **not** propagated: it does not emit `session.error`, does not `mark_error`, and is not stored in `auto_sync_failure` (so the final `propagate_failures` re-raise never sees it). The dispatcher therefore records `task.completed` for the primary work that did succeed.

**`agent.autosync.rate_limited`.** The condition is surfaced as a distinct, non-terminal `agent.autosync.rate_limited {reason}` event in the `agent.*` namespace — not `session.error` (which mid-2026's UIs render as a terminal failure) and not a `task.*` event (the task did complete). This keeps hard rule 1 intact: Mad does not interpret the auto-sync output or manage its retry; a best-effort publish step that hit a quota is just reported, not re-driven.

**Scope.** Only the auto-sync `RateLimitError` is reclassified. A `RateLimitError` from the **primary** run still re-raises cleanly into the dispatcher's retry loop (that retry re-runs the primary, which is correct — the primary has *not* succeeded). Any **non**-rate-limit auto-sync failure keeps its prior behavior: `session.error` + `mark_error` + re-raise under `propagate_failures` (→ terminal `task.failed`). Retrying only the auto-sync portion (rather than treating it as best-effort) was considered and rejected for v1: it would entangle the dispatcher's single retry loop with two distinct run phases for no operator-visible gain over a clearly-surfaced non-terminal event. The fire-and-forget `/messages` path (`propagate_failures=False`) never re-ran the primary, but it too now emits `agent.autosync.rate_limited` instead of a misleading `session.error`.
